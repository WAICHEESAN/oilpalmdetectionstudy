...
"""
# DenseNet121 base + K-fold + flatten + softmax
"""
epochs = 10
batch_size = 32
num_folds = 10
...
for train, test in kfold.split(train_features, train_labels):
    ...
    dense_soft_flat.add(Dropout(0.5))
    dense_soft_flat.add(layers.Dense(2, activation = 'softmax'))
    dense_soft_flat.summary()
    # compile model
    dense_soft_flat.compile(optimizer = optimizers.RMSprop(learning_rate=0.0001),
...
dense_soft_flat.save('dense_soft_flat.h5')
...
dense_soft_flat = tf.keras.models.load_model('dense_soft_flat.h5')
...
plot_confusion_matrix(cm=cm_dense_flat, classes=labels, title='DenseNet-Flatten-Softmax')
plt.tight_layout()
plt.savefig('dense_soft_flat.png')
plt.clf()