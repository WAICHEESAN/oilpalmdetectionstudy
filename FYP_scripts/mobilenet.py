# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 06:06:43 2021
...
# Define feature extractor for MobileNet model   
def features_mobile(directory, sample_count, img_width, img_height, outputdim, num_classes):
...
train_features, train_labels = features_mobile(train_path, train_size, 224, 224, 7, 2)
test_features, test_labels = features_mobile(test_path, test_size, 224, 224, 7, 2)

"""
MobileNet + 10 KFold + Flatten + Softmax
"""
epochs = 10
batch_size = 32
num_folds = 10
...
for train, test in kfold.split(train_features, train_labels):
    ...
    mobile_soft_flat.add(layers.Dense(2, activation = 'softmax'))
    mobile_soft_flat.summary()
    # compile model
    mobile_soft_flat.compile(optimizer = optimizers.RMSprop(learning_rate=0.0001),
...
mobile_soft_flat.save('mobile_soft_flat.h5')
...
mobile_soft_flat = tf.keras.models.load_model('mobile_soft_flat.h5')
...
plot_confusion_matrix(cm=cm_mobile_flat, classes=labels, title='MobileNet-Flatten-Softmax')
plt.tight_layout()
plt.savefig('mobile_soft_flat.png')
plt.clf()
